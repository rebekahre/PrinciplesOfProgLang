...
construct_while : 
    T_WHILE 
    {
      ...
      // TODO: Store the next instruction entry in the parser's
      // stack
    ...
    T_RPAR 
    {
      ...
      // TODO: Jump to the end of the while body if the 
      // condition 
      // evaluates to zero.
    ...
    stmt
    {
      ...
      // TODO: generate an unconditional jump to the first 
      // instruction of l_expr
...
construct_repeat: 
    ...
    {
      // First semantic action
      // TODO: store in the stack the entry number of the next
      // instruction to be generated (use @$.begin.line 
      // instead of $$)
      @$.begin.line = INSTRUCTION_NEXT;
    }
    stmt_list 
    ...
    {
      // Second semantic action.
      // TODO: Retrieve the value stored in the stack in the 
      //       first semantic action above (the second symbol)
      int jump_dst = @2.begin.line;
      // TODO: Generate a jump-if-zero (OP_JZ) to the address 
      // stored in the first semantic action of this rule
...
construct_if :
    ...
    T_RPAR 
    {
      ...
      // TBDARG = Should modify the corresponding address 
      //	  (.addr#) in a later semantic action.
      // NOARG = No need to change.
      itab_instruction_add (itab, OP_JZ, $3->addr, NOARG, TBD_ARG);
    ...
    stmt 
    {
      ...
      // INSTRUCTION_NEXT or INSTRUCTION_LAST
      @$.begin.line =  INSTRUCTION_LAST; 
...
assignment : T_ID arr_index T_ASSIGN a_expr 
    {
      ...
      {
        // Want the type of the intermediate casting variable 
        // to be the same as the left-hand-side.
        temp = make_temp (symtab, sym->datatype);
        // TASK: Complete the four TBD_ARG in both calls to 
        // itab_instruction_add.
        ...
        // Final store to the array will use the intermediate 
        // variable resulting from the cast.
        src_temp = temp;
      }
      ...
      {
        int opcode;
        // Decide the operation code for loading the array 
        // entry into the temporary variable.
        ...
        // Decide: the target address (first argument after 
        // opcode), the base address of the array to load from 
        // (second argument after opcode), and the offset 
        // address (the index) to the array (third argument 
        // after opcode).

        // TASK: Complete the two TBD_ARG in the following call
        //       to itab_instruction_add.
        // HINT: See the code corresponding to 
	// OP_LOAD_ARRAY_VAL_*
        itab_instruction_add (itab, opcode, sym->addr, $2->addr, src_temp->addr);
      }
      else
      {
        // This is what we were doing previously in assignment
	// 3.
...
a_expr : a_expr T_ADD a_term 
      {
        ...
        // TASK: Abort if the datatype of a_expr and a_term 
	// differ.
        symbol_t * res;
        ...
        {
          // TASK: Modify this semantic action to support both
	  //       DTYPE_INT and DTYPE_FLOAT.
          // For DTYPE_FLOAT you should generate an OP_FADD
	  // instruction.
          res = make_temp (symtab, $1->datatype);
    ...
    | a_expr T_SUB a_term
      {
        // TASK: Abort if the datatype of a_expr and a_term
	//       differ.
        // TASK: Complete support for OP_SUB and OP_FSUB. 
	//       See OP_ADD and OP_FADD code above.
        symbol_t * res;
        if ($1->datatype == DTYPE_INT)
        {
          res = make_temp (symtab, $1->datatype);
        ...
        if ($1->datatype == DTYPE_FLOAT)
        {
          // TASK: Modify this semantic action to support both
	  //       DTYPE_INT and DTYPE_FLOAT.
          // For DTYPE_FLOAT you should generate an OP_FSUB 
	  // instruction.
          res = make_temp (symtab, $1->datatype);
...
a_term : a_term T_MUL a_fact
      {
        // TASK: Abort if the datatype of a_expr and a_term 
	//       differ.
        // TASK: Complete support for OP_SUB and OP_FSUB. See 
	//       OP_ADD and OP_FADD code above.
        symbol_t * res;
        if ($1->datatype == DTYPE_INT)
        {
          res = make_temp (symtab, $1->datatype);
        ...
        if ($1->datatype == DTYPE_FLOAT)
        {
          // TASK: Modify this semantic action to support both
	  //       DTYPE_INT and DTYPE_FLOAT.
          // For DTYPE_FLOAT you should generate an OP_FMUL 
	  // instruction.
          res = make_temp (symtab, $1->datatype);
	...
	//       OP_ADD and OP_FADD code above.
        ...
        if ($1->datatype == DTYPE_FLOAT)
        {
          // TASK: Modify this semantic action to support both 
	  //       DTYPE_INT and DTYPE_FLOAT.
          // For DTYPE_FLOAT you should generate an OP_FDIV 
	  // instruction.
          res = make_temp (symtab, $1->datatype);
...
a_fact : varref 
    ...
    | T_FLOAT 
      { 
        // TASK: Complete implementation in a fashion similar 
	//       to the rule a_fact -> T_INTEGER. 
        symbol_t * res;
